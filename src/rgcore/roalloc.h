...
//
#define roalloc(sz) _roalloc(sz, __FILE__, __LINE__, __FUNCTION__, NULL)
#define rorealloc(oldptr, newsz) _rorealloc(oldptr, newsz, __FILE__, __LINE__, __FUNCTION__, NULL)
...
// Normal Version
//
#define roalloc(sz) _roalloc(sz)
#define rorealloc(oldptr, newsz) _rorealloc(oldptr, newsz)
#define rofree(ptr) _rofree(ptr) //ptr = NULL
#define rostrdup(pcstring) _rostrdup(pcstring)
void *_roalloc(size_t sz);
void *_rorealloc(void *oldptr, size_t newsz);
...
#if ROALLOC_TRACE
		void *ptr = _roalloc(size, __FILE__, __LINE__, __FUNCTION__, _ReturnAddress() );
#else
		void *ptr = roalloc(size);
#endif
		if(ptr == NULL) {
