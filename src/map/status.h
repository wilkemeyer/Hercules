...
public:
	...
	/* vars */
	static int current_equip_item_index;
	static int current_equip_card_id;

	static struct s_status_dbs *dbs;

	static struct eri *data_ers; //For sc_data entries
	static struct status_data dummy;
	...
	/* funcs */
	static int get_refine_chance (enum refine_type wlv, int refine);
	// for looking up associated data
	static sc_type skill2sc (int skill_id);
	static int sc2skill (sc_type sc);
	static unsigned int sc2scb_flag (sc_type sc);
	static int type2relevant_bl_types (int type);
	static int get_sc_type (sc_type idx);
	static int damage (struct block_list *src,struct block_list *target,int64 hp,int64 sp, int walkdelay, int flag);
	//Define for standard HP/SP skill-related cost triggers (mobs require no HP/SP to use skills)
	static int charge (struct block_list* bl, int64 hp, int64 sp);
	static int percent_change (struct block_list *src,struct block_list *target,signed char hp_rate, signed char sp_rate, int flag);
	//Used to set the hp/sp of an object to an absolute value (can't kill)
	static int set_hp (struct block_list *bl, unsigned int hp, int flag);
	static int set_sp (struct block_list *bl, unsigned int sp, int flag);
	static int heal (struct block_list *bl,int64 hp,int64 sp, int flag);
	static int revive (struct block_list *bl, unsigned char per_hp, unsigned char per_sp);
	static int fixed_revive (struct block_list *bl, unsigned int per_hp, unsigned int per_sp);
	static struct regen_data * get_regen_data (struct block_list *bl);
	static struct status_data * get_status_data (struct block_list *bl);
	static struct status_data * get_base_status (struct block_list *bl);
	static const char * get_name (struct block_list *bl);
	static int get_class (struct block_list *bl);
	static int get_lv (struct block_list *bl);
	static defType get_def (struct block_list *bl);
	static unsigned short get_speed (struct block_list *bl);
	static unsigned char calc_attack_element (struct block_list *bl, struct status_change *sc, int element);
	static int get_party_id (struct block_list *bl);
	static int get_guild_id (struct block_list *bl);
	static int get_emblem_id (struct block_list *bl);
	static int get_mexp (struct block_list *bl);
	static int get_race2 (struct block_list *bl);
	static struct view_data * get_viewdata (struct block_list *bl);
	static void set_viewdata (struct block_list *bl, int class_);
	static void change_init (struct block_list *bl);
	static struct status_change * get_sc (struct block_list *bl);
	static int isdead (struct block_list *bl);
	static int isimmune (struct block_list *bl);
	static int get_sc_def (struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int tick, int flag);
	static int change_start (struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4, int tick, int flag);
	static int change_end_ (struct block_list* bl, enum sc_type type, int tid, const char* file, int line);
	static int kaahi_heal_timer (int tid, int64 tick, int id, intptr_t data);
	static int change_timer (int tid, int64 tick, int id, intptr_t data);
	static int change_timer_sub (struct block_list* bl, va_list ap);
	static int change_clear (struct block_list* bl, int type);
	static int change_clear_buffs (struct block_list* bl, int type);
	static void calc_bl_ (struct block_list *bl, enum scb_flag flag, enum e_status_calc_opt opt);
	static int calc_mob_ (struct mob_data* md, enum e_status_calc_opt opt);
	static int calc_pet_ (struct pet_data* pd, enum e_status_calc_opt opt);
	static int calc_pc_ (struct map_session_data* sd, enum e_status_calc_opt opt);
	static void calc_pc_additional (struct map_session_data* sd, enum e_status_calc_opt opt);
	static int calc_homunculus_ (struct homun_data *hd, enum e_status_calc_opt opt);
	static int calc_mercenary_ (struct mercenary_data *md, enum e_status_calc_opt opt);
	static int calc_elemental_ (struct elemental_data *ed, enum e_status_calc_opt opt);
	static void calc_misc (struct block_list *bl, struct status_data *st, int level);
	static void calc_regen (struct block_list *bl, struct status_data *st, struct regen_data *regen);
	static void calc_regen_rate (struct block_list *bl, struct regen_data *regen, struct status_change *sc);
	static int check_skilluse (struct block_list *src, struct block_list *target, uint16 skill_id, int flag); // [Skotlex]
	static int check_visibility (struct block_list *src, struct block_list *target); //[Skotlex]
	static int change_spread (struct block_list *src, struct block_list *bl);
	static defType calc_def (struct block_list *bl, struct status_change *sc, int def, bool viewable);
	static short calc_def2 (struct block_list *bl, struct status_change *sc, int def2, bool viewable);
	static defType calc_mdef (struct block_list *bl, struct status_change *sc, int mdef, bool viewable);
	static short calc_mdef2 (struct block_list *bl, struct status_change *sc, int mdef2, bool viewable);
	static unsigned short calc_batk(struct block_list *bl, struct status_change *sc, int batk, bool viewable);
	static unsigned short base_matk (struct block_list *bl, const struct status_data *st, int level);
	static int get_weapon_atk (struct block_list *src, struct weapon_atk *watk, int flag);
	static int get_total_mdef (struct block_list *src);
	static int get_total_def (struct block_list *src);
	static int get_matk (struct block_list *src, int flag);
	static void update_matk ( struct block_list *bl );
	static int readdb (void);

	static void initChangeTables (void);
	static void initDummyData (void);
	static int base_amotion_pc (struct map_session_data *sd, struct status_data *st);
	static unsigned short base_atk (const struct block_list *bl, const struct status_data *st);
	static unsigned int get_base_maxhp (struct map_session_data *sd, struct status_data *st);
	static unsigned int get_base_maxsp (struct map_session_data *sd, struct status_data *st);
	static int calc_npc_ (struct npc_data *nd, enum e_status_calc_opt opt);
	static unsigned short calc_str (struct block_list *bl, struct status_change *sc, int str);
	static unsigned short calc_agi (struct block_list *bl, struct status_change *sc, int agi);
	static unsigned short calc_vit (struct block_list *bl, struct status_change *sc, int vit);
	static unsigned short calc_int (struct block_list *bl, struct status_change *sc, int int_);
	static unsigned short calc_dex (struct block_list *bl, struct status_change *sc, int dex);
	static unsigned short calc_luk (struct block_list *bl, struct status_change *sc, int luk);
	static unsigned short calc_watk (struct block_list *bl, struct status_change *sc, int watk, bool viewable);
	static unsigned short calc_matk (struct block_list *bl, struct status_change *sc, int matk, bool viewable);
	static signed short calc_hit (struct block_list *bl, struct status_change *sc, int hit, bool viewable);
	static signed short calc_critical (struct block_list *bl, struct status_change *sc, int critical, bool viewable);
	static signed short calc_flee (struct block_list *bl, struct status_change *sc, int flee, bool viewable);
	static signed short calc_flee2 (struct block_list *bl, struct status_change *sc, int flee2, bool viewable);
	static unsigned short calc_speed (struct block_list *bl, struct status_change *sc, int speed);
	static short calc_aspd_rate (struct block_list *bl, struct status_change *sc, int aspd_rate);
	static unsigned short calc_dmotion (struct block_list *bl, struct status_change *sc, int dmotion);
	static short calc_aspd (struct block_list *bl, struct status_change *sc, short flag);
	static short calc_fix_aspd (struct block_list *bl, struct status_change *sc, int aspd);
	static unsigned int calc_maxhp (struct block_list *bl, struct status_change *sc, uint64 maxhp);
	static unsigned int calc_maxsp (struct block_list *bl, struct status_change *sc, unsigned int maxsp);
	static unsigned char calc_element (struct block_list *bl, struct status_change *sc, int element);
	static unsigned char calc_element_lv (struct block_list *bl, struct status_change *sc, int lv);
	static unsigned short calc_mode (struct block_list *bl, struct status_change *sc, int mode);
	static unsigned short calc_ematk (struct block_list *bl, struct status_change *sc, int matk);
	static void calc_bl_main (struct block_list *bl, int flag);
	static void display_add (struct map_session_data *sd, enum sc_type type, int dval1, int dval2, int dval3);
	static void display_remove (struct map_session_data *sd, enum sc_type type);
	static int natural_heal (struct block_list *bl, va_list args);
	static int natural_heal_timer (int tid, int64 tick, int id, intptr_t data);
	static bool readdb_job2 (char *fields[], int columns, int current);
	static bool readdb_sizefix (char *fields[], int columns, int current);
	static int readdb_refine_libconfig (const char *filename);
	static int readdb_refine_libconfig_sub (config_setting_t *r, const char *name, const char *source);
	static bool readdb_scconfig (char *fields[], int columns, int current);
	static void read_job_db (void);
	static void read_job_db_sub (int idx, const char *name, config_setting_t *jdb);
};
