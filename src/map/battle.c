	...
	if (tsc && tsc->count) {
		...
		if( tsc->data[SC_POISONINGWEAPON] ) {
			...
			if ( !(flag&BF_SKILL) && (flag&BF_WEAPON) && damage > 0 && rnd()%100 < tsc->data[SC_POISONINGWEAPON]->val3 ) {
				...
				sc_start(src,bl,(sc_type) tsc->data[SC_POISONINGWEAPON]->val2,rate,tsc->data[SC_POISONINGWEAPON]->val1,skill->get_time2(GC_POISONINGWEAPON,1) - (tstatus->vit + tstatus->luk) / 2 * 1000);
			}
		}
...
void CBattle::consume_ammo(struct map_session_data *sd, int skill_id, int lv)
{
	int qty=1;
...
void CBattle::drain(struct map_session_data *sd, struct block_list *tbl, int64 rdamage, int64 ldamage, int race, int boss)
{
