...
public:
	...
	static struct mob_db *db_data[MAX_MOB_DB + 1];
	static struct mob_db *dummy; //Dummy mob to be returned when a non-existant one is requested.
	// Dynamic mob chat database
	static struct mob_chat *chat_db[MAX_MOB_CHAT + 1];
	// Defines the Manuk/Splendide/Mora mob groups for the status reductions [Epoque & Frost]
	static int manuk[8];
	static int splendide[5];
	static int mora[5];
	/* */
	static int init (bool mimimal);
	static int final (void);
	static void reload (void);
	/* */
	static struct mob_db* db (int index);
	static struct mob_chat* chat (short id);
	static int makedummymobdb (int);
	static int spawn_guardian_sub (int tid, int64 tick, int id, intptr_t data);
	static int skill_id2skill_idx (int class_, uint16 skill_id);
	static int db_searchname (const char *str);
	static int db_searchname_array_sub (struct mob_db *monster, const char *str, int flag);
	// MvP Tomb System
	static void mvptomb_create (struct mob_data *md, char *killer, time_t time);
	static void mvptomb_destroy (struct mob_data *md);
	static int db_searchname_array (struct mob_db **data, int size, const char *str, int flag);
	static int db_checkid (const int id);
	static struct view_data* get_viewdata (int class_);
	static int parse_dataset (struct spawn_data *data);
	static struct mob_data* spawn_dataset (struct spawn_data *data);
	static int get_random_id (int type, int flag, int lv);
	static bool ksprotected (struct block_list *src, struct block_list *target);
	static struct mob_data* once_spawn_sub (struct block_list *bl, int16 m, int16 x, int16 y, const char *mobname, int class_, const char *event, unsigned int size, unsigned int ai);
	static int once_spawn (struct map_session_data *sd, int16 m, int16 x, int16 y, const char *mobname, int class_, int amount, const char *event, unsigned int size, unsigned int ai);
	static int once_spawn_area (struct map_session_data *sd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, const char *mobname, int class_, int amount, const char *event, unsigned int size, unsigned int ai);
	static int spawn_guardian (const char *mapname, short x, short y, const char *mobname, int class_, const char *event, int guardian, bool has_index);
	static int spawn_bg (const char *mapname, short x, short y, const char *mobname, int class_, const char *event, unsigned int bg_id);
	static int can_reach (struct mob_data *md, struct block_list *bl, int range, int state);
	static int linksearch (struct block_list *bl, va_list ap);
	static int delayspawn (int tid, int64 tick, int id, intptr_t data);
	static int setdelayspawn (struct mob_data *md);
	static int count_sub (struct block_list *bl, va_list ap);
	static int spawn (struct mob_data *md);
	static int can_changetarget (struct mob_data *md, struct block_list *target, int mode);
	static int target (struct mob_data *md, struct block_list *bl, int dist);
	...
	static int ai_sub_hard_slavemob (struct mob_data *md, int64 tick);
	static int unlocktarget (struct mob_data *md, int64 tick);
	static int randomwalk (struct mob_data *md, int64 tick);
	static int warpchase (struct mob_data *md, struct block_list *target);
	static bool ai_sub_hard (struct mob_data *md, int64 tick);
	static int ai_sub_hard_timer (struct block_list *bl, va_list ap);
	static int ai_sub_foreachclient (struct map_session_data *sd, va_list ap);
	static int ai_sub_lazy (struct mob_data *md, va_list args);
	static int ai_lazy (int tid, int64 tick, int id, intptr_t data);
	static int ai_hard (int tid, int64 tick, int id, intptr_t data);
	static struct item_drop* setdropitem (int nameid, int qty, struct item_data *data);
	static struct item_drop* setlootitem (struct item *item);
	static int delay_item_drop (int tid, int64 tick, int id, intptr_t data);
	static void item_drop (struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, unsigned short flag);
	static int timer_delete (int tid, int64 tick, int id, intptr_t data);
	static int deleteslave_sub (struct block_list *bl, va_list ap);
	static int deleteslave (struct mob_data *md);
	static int respawn (int tid, int64 tick, int id, intptr_t data);
	static void log_damage (struct mob_data *md, struct block_list *src, int damage);
	static void damage (struct mob_data *md, struct block_list *src, int damage);
	static int dead (struct mob_data *md, struct block_list *src, int type);
	static void revive (struct mob_data *md, unsigned int hp);
	static int guardian_guildchange (struct mob_data *md);
	static int random_class (int *value, size_t count);
	static int class_change (struct mob_data *md, int class_);
	static void heal (struct mob_data *md, unsigned int heal);
	static int warpslave_sub (struct block_list *bl, va_list ap);
	static int warpslave (struct block_list *bl, int range);
	static int countslave_sub (struct block_list *bl, va_list ap);
	static int countslave (struct block_list *bl);
	static int summonslave (struct mob_data *md2, int *value, int amount, uint16 skill_id);
	static int getfriendhprate_sub (struct block_list *bl, va_list ap);
	static struct block_list* getfriendhprate (struct mob_data *md, int min_rate, int max_rate);
	static struct block_list* getmasterhpltmaxrate (struct mob_data *md, int rate);
	static int getfriendstatus_sub (struct block_list *bl, va_list ap);
	static struct mob_data* getfriendstatus (struct mob_data *md, int cond1, int cond2);
	static int skill_use (struct mob_data *md, int64 tick, int event);
	static int skill_event (struct mob_data *md, struct block_list *src, int64 tick, int flag);
	static int is_clone (int class_);
	static int clone_spawn (struct map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, int mode, int flag, unsigned int duration);
	static int clone_delete (struct mob_data *md);
	...
	static bool get_const (const config_setting_t *it, int *value);
	static int read_libconfig (const char *filename, bool ignore_missing);
	static void read_db_additional_fields (struct mob_db *entry, int class_, config_setting_t *it, int n, const char *source);
	static bool read_db_sub (config_setting_t *mobt, int id, const char *source);
	static void read_db_drops_sub (struct mob_db *entry, struct status_data *mstatus, int class_, config_setting_t *t);
	static void read_db_mvpdrops_sub (struct mob_db *entry, struct status_data *mstatus, int class_, config_setting_t *t);
	static int read_db_mode_sub (struct mob_db *entry, struct status_data *mstatus, int class_, config_setting_t *t);
	static void read_db_stats_sub (struct mob_db *entry, struct status_data *mstatus, int class_, config_setting_t *t);
	static void name_constants (void);
	static bool readdb_mobavail (char *str[], int columns, int current);
	static int read_randommonster (void);
	static bool parse_row_chatdb (char **str, const char *source, int line, int *last_msg_id);
	static void readchatdb (void);
	static bool parse_row_mobskilldb (char **str, int columns, int current);
	static void readskilldb (void);
