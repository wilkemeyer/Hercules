...
void mob_read_db_drops_sub(struct mob_db *entry, config_setting_t *t)
{
	...
	while (idx < MAX_MOB_DROP && (drop = libconfig->setting_get_elem(t, i))) {
		...
		type = id->type;
		if ((entry->mob_id >= 1324 && entry->mob_id <= 1363) || (entry->mob_id >= 1938 && entry->mob_id <= 1946)) { // FIXME: Constants
		...
		if (entry->dropitem[idx].p && (entry->mob_id < 1324 || entry->mob_id > 1363) && (entry->mob_id < 1938 || entry->mob_id > 1946)) {
