...
public:
	...
	/* vars */
	static bool minimal;     ///< Starts the server in minimal initialization mode.
	static bool scriptcheck; ///< Starts the server in script-check mode.

	/** Additional scripts requested through the command-line */
	static char **extra_scripts;
	static int extra_scripts_count;

	static int retval;
	static int count;

	static int autosave_interval;
	static int minsave_interval;
	static int save_settings;
	static int agit_flag;
	static int agit2_flag;
	static int night_flag; // 0=day, 1=night [Yor]
	static int enable_spy; //Determines if @spy commands are active.
	static char db_path[256];

	static char help_txt[256];
	static char help2_txt[256];
	static char charhelp_txt[256];

	static char wisp_server_name[NAME_LENGTH];

	static char *INTER_CONF_NAME;
	static char *LOG_CONF_NAME;
	static char *MAP_CONF_NAME;
	static char *BATTLE_CONF_FILENAME;
	static char *ATCOMMAND_CONF_FILENAME;
	static char *SCRIPT_CONF_NAME;
	static char *MSG_CONF_NAME;
	static char *GRF_PATH_FILENAME;

	static char autotrade_merchants_db[32];
	static char autotrade_data_db[32];
	static char npc_market_data_db[32];

	static char default_codepage[32];
	static char default_lang_str[64];
	static uint8 default_lang_id;

	static int server_port;
	static char server_ip[32];
	static char server_id[32];
	static char server_pw[100];
	static char server_db[32];
	static Sql* mysql_handle;

	static int port;
	static int users;
	static int enable_grf; //To enable/disable reading maps from GRF files, bypassing mapcache [blackhole89]
	static bool ip_set;
	static bool char_ip_set;

	static int16 index2mapid[MAX_MAPINDEX];
	/* */
	static DBMap* id_db;     // int id -> struct block_list*
	static DBMap* pc_db;     // int id -> struct map_session_data*
	static DBMap* mobid_db;  // int id -> struct mob_data*
	static DBMap* bossid_db; // int id -> struct mob_data* (MVP db)
	static DBMap* map_db;    // unsigned int mapindex -> struct map_data_other_server*
	static DBMap* nick_db;   // int char_id -> struct charid2nick* (requested names of offline characters)
	static DBMap* charid_db; // int char_id -> struct map_session_data*
	static DBMap* regen_db;  // int id -> struct block_list* (status_natural_heal processing)
	static DBMap* zone_db;   // string => struct map_zone_data
	static DBMap* iwall_db;
	static struct block_list **block_free;
	static int block_free_count, block_free_lock, block_free_list_size;
	static struct block_list **bl_list;
	static int bl_list_count, bl_list_size;
	
	// This block is zeroed in map_defaults()
	static struct block_list bl_head;
	static struct map_zone_data zone_all;/* used as a base on all maps */
	static struct map_zone_data zone_pk;/* used for (pk_mode) */
	// End of zeroed block

	/* */
	static struct map_session_data *cpsd;
	static struct map_data *list;
	/* [Ind/Hercules] */
	static struct eri *iterator_ers;
	static char *cache_buffer; // Has the uncompressed gat data of all maps, so just one allocation has to be made
	/* */
	static struct eri *flooritem_ers;
	/* */
	static int bonus_id;
	/* */
	static bool cpsd_active;

	/* funcs */
	static void zone_init (void);
	static void zone_remove (int m);
	static void zone_apply (int m, struct map_zone_data *zone, const char* start, const char* buffer, const char* filepath);
	static void zone_change (int m, struct map_zone_data *zone, const char* start, const char* buffer, const char* filepath);
	static void zone_change2 (int m, struct map_zone_data *zone);

	static int getcell (int16 m, const struct block_list *bl, int16 x, int16 y, cell_chk cellchk);
	static void setgatcell (int16 m, int16 x, int16 y, int gat);

	static void cellfromcache (struct map_data *m);
	// users
	static void setusers (int);
	static int getusers (void);
	static int usercount (void);
	// blocklist lock
	static int freeblock (struct block_list *bl);
	static int freeblock_lock (void);
	static int freeblock_unlock (void);
	// blocklist manipulation
	static int addblock (struct block_list* bl);
	static int delblock (struct block_list* bl);
	static int moveblock (struct block_list *bl, int x1, int y1, int64 tick);
	//blocklist nb in one cell
	static int count_oncell (int16 m,int16 x,int16 y,int type,int flag);
	static struct skill_unit * find_skill_unit_oncell (struct block_list* target,int16 x,int16 y,uint16 skill_id,struct skill_unit* out_unit, int flag);
	// search and creation
	static int get_new_object_id (void);
	static int search_freecell (struct block_list *src, int16 m, int16 *x, int16 *y, int16 rx, int16 ry, int flag);
	static bool closest_freecell (int16 m, const struct block_list *bl, int16 *x, int16 *y, int type, int flag);
	//
	static int quit (struct map_session_data *sd);
	// npc
	static bool addnpc (int16 m,struct npc_data *nd);
	// map item
	static int clearflooritem_timer (int tid, int64 tick, int id, intptr_t data);
	static int removemobs_timer (int tid, int64 tick, int id, intptr_t data);
	static void clearflooritem (struct block_list* bl);
	static int addflooritem (const struct block_list *bl, struct item *item_data, int amount, int16 m, int16 x, int16 y, int first_charid, int second_charid, int third_charid, int flags);
	// player to map session
	static void addnickdb (int charid, const char* nick);
	static void delnickdb (int charid, const char* nick);
	static void reqnickdb (struct map_session_data* sd,int charid);
	static const char* charid2nick (int charid);
	static struct map_session_data* charid2sd (int charid);

	static void vforeachpc (int (*func)(struct map_session_data* sd, va_list args), va_list args);
	static void foreachpc (int (*func)(struct map_session_data* sd, va_list args), ...);
	static void vforeachmob (int (*func)(struct mob_data* md, va_list args), va_list args);
	static void foreachmob (int (*func)(struct mob_data* md, va_list args), ...);
	static void vforeachnpc (int (*func)(struct npc_data* nd, va_list args), va_list args);
	static void foreachnpc (int (*func)(struct npc_data* nd, va_list args), ...);
	static void vforeachregen (int (*func)(struct block_list* bl, va_list args), va_list args);
	static void foreachregen (int (*func)(struct block_list* bl, va_list args), ...);
	static void vforeachiddb (int (*func)(struct block_list* bl, va_list args), va_list args);
	static void foreachiddb (int (*func)(struct block_list* bl, va_list args), ...);

	static int vforeachinrange (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int type, va_list ap);
	static int foreachinrange (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int type, ...);
	static int vforeachinshootrange (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int type, va_list ap);
	static int foreachinshootrange (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int type, ...);
	static int vforeachinarea (int (*func)(struct block_list*,va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int type, va_list ap);
	static int foreachinarea (int (*func)(struct block_list*,va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int type, ...);
	static int vforcountinrange (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int count, int type, va_list ap);
	static int forcountinrange (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int count, int type, ...);
	static int vforcountinarea (int (*func)(struct block_list*,va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int count, int type, va_list ap);
	static int forcountinarea (int (*func)(struct block_list*,va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int count, int type, ...);
	static int vforeachinmovearea (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int16 dx, int16 dy, int type, va_list ap);
	static int foreachinmovearea (int (*func)(struct block_list*,va_list), struct block_list* center, int16 range, int16 dx, int16 dy, int type, ...);
	static int vforeachincell (int (*func)(struct block_list*,va_list), int16 m, int16 x, int16 y, int type, va_list ap);
	static int foreachincell (int (*func)(struct block_list*,va_list), int16 m, int16 x, int16 y, int type, ...);
	static int vforeachinpath (int (*func)(struct block_list*,va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int16 range, int length, int type, va_list ap);
	static int foreachinpath (int (*func)(struct block_list*,va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int16 range, int length, int type, ...);
	static int vforeachinmap (int (*func)(struct block_list*,va_list), int16 m, int type, va_list args);
	static int foreachinmap (int (*func)(struct block_list*,va_list), int16 m, int type, ...);
	...
	static int mapname2ipport (unsigned short name, uint32* ip, uint16* port);
	static int setipport (unsigned short map_index, uint32 ip, uint16 port);
	static int eraseipport (unsigned short map_index, uint32 ip, uint16 port);
	...
	static struct mob_data * id2boss (int id);
	static uint32 race_id2mask (int race);
	// reload config file looking only for npcs
	static void reloadnpc (bool clear);

	static int check_dir (int s_dir,int t_dir);
	static uint8 calc_dir (struct block_list *src,int16 x,int16 y);
	static int random_dir (struct block_list *bl, short *x, short *y); // [Skotlex]

	static int cleanup_sub (struct block_list *bl, va_list ap);

	static int delmap (char* mapname);
	static void flags_init (void);

	static bool iwall_set (int16 m, int16 x, int16 y, int size, int8 dir, bool shootable, const char* wall_name);
	static void iwall_get (struct map_session_data *sd);
	static void iwall_remove (const char *wall_name);

	static int addmobtolist (unsigned short m, struct spawn_data *spawn); // [Wizputer]
	static void spawnmobs (int16 m); // [Wizputer]
	static void removemobs (int16 m); // [Wizputer]
	//void (*do_reconnect_map) (void); //Invoked on map-char reconnection [Skotlex] Note used but still keeping it, just in case
	static void addmap2db (struct map_data *m);
	static void removemapdb (struct map_data *m);
	static void clean (int i);

	static void do_shutdown (void);

	static int freeblock_timer (int tid, int64 tick, int id, intptr_t data);
	static int searchrandfreecell (int16 m, const struct block_list *bl, int16 *x, int16 *y, int stack);
	static int count_sub (struct block_list *bl, va_list ap);
	static DBData create_charid2nick (DBKey key, va_list args);
	static int removemobs_sub (struct block_list *bl, va_list ap);
	static struct mapcell gat2cell (int gat);
	static int cell2gat (struct mapcell cell);
	static int getcellp (struct map_data *m, const struct block_list *bl, int16 x, int16 y, cell_chk cellchk);
	static void setcell (int16 m, int16 x, int16 y, cell_t cell, bool flag);
	static int sub_getcellp (struct map_data *m, const struct block_list *bl, int16 x, int16 y, cell_chk cellchk);
	static void sub_setcell (int16 m, int16 x, int16 y, cell_t cell, bool flag);
	static void iwall_nextxy (int16 x, int16 y, int8 dir, int pos, int16 *x1, int16 *y1);
	static DBData create_map_data_other_server (DBKey key, va_list args);
	static int eraseallipport_sub (DBKey key, DBData *data, va_list va);
	static char* init_mapcache (FILE *fp);
	static int readfromcache (struct map_data *m, char *buffer);
	static int addmap (const char *mapname);
	static void delmapid (int id);
	static void zone_db_clear (void);
	static void list_final (void);
	static int waterheight (char *mapname);
	static int readgat (struct map_data *m);
	static int readallmaps (void);
	static int config_read (char *cfgName);
	static int config_read_sub (char *cfgName);
	static void reloadnpc_sub (char *cfgName);
	static int inter_config_read (char *cfgName);
	static int sql_init (void);
	static int sql_close (void);
	static bool zone_mf_cache (int m, char *flag, char *params);
	static unsigned short zone_str2itemid (const char *name);
	static unsigned short zone_str2skillid (const char *name);
	static enum bl_type zone_bl_type (const char *entry, enum map_zone_skill_subtype *subtype);
	static void read_zone_db (void);
	static int db_final (DBKey key, DBData *data, va_list ap);
	static int nick_db_final (DBKey key, DBData *data, va_list args);
	static int cleanup_db_sub (DBKey key, DBData *data, va_list va);
	static int abort_sub (struct map_session_data *sd, va_list ap);
	static void update_cell_bl (struct block_list *bl, bool increase);
	static int get_new_bonus_id (void);
	static void add_questinfo (int m, struct questinfo *qi);
	static bool remove_questinfo (int m, struct npc_data *nd);
	static struct map_zone_data *merge_zone (struct map_zone_data *main, struct map_zone_data *other);
	static void zone_clear_single (struct map_zone_data *zone);
};
extern CMap *map;
